"""Live, read-only view of the serious Alpaca PAPER account, rendered server-side.

On the public deploy the locally-generated reports/dashboard.html doesn't exist,
so the Dashboard tab falls back to this: the current paper-account equity, day
P&L, and open positions, fetched live from Alpaca and shown behind the login.

HARD-LOCKED to the PAPER endpoint — this module can only READ a paper account.
It never places an order and never touches the live-money API. It reads only the
serious account's keys (ALPACA_API_KEY_ID / ALPACA_API_SECRET_KEY); the casino
(ALPACA_DEGEN) account is intentionally NOT surfaced here.
"""
...
_cache: dict = {}           # {"ts": monotonic, "data": dict}


def fetch_account() -> dict:
    """Return {available, account, positions, error}. Never raises — the page
    degrades to a friendly message on missing keys or any API hiccup."""
 ...
 *{box-sizing:border-box}
 ...
 .hero{background:var(--surface);border:1px solid var(--line);border-radius:14px;padding:1.2rem 1.3rem;margin-bottom:1rem}
 .eq{font-size:34px;font-weight:700;letter-spacing:-.01em;font-variant-numeric:tabular-nums}
 .eqlabel{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em}
 .day{font-size:15px;font-weight:600;margin-top:.2rem;font-variant-numeric:tabular-nums}
 .cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:.7rem;margin:1rem 0}
 .mc{background:var(--surface);border:1px solid var(--line);border-radius:12px;padding:.7rem .85rem}
 .mc .l{font-size:11.5px;color:var(--muted)}
 .mc .v{font-size:18px;font-weight:600;margin-top:2px;font-variant-numeric:tabular-nums}
 h2{font-size:15px;font-weight:600;margin:1.4rem 0 .5rem}
 table{width:100%;border-collapse:collapse;font-size:13px}
 th,td{text-align:right;padding:.45rem .5rem;border-bottom:1px solid var(--line);font-variant-numeric:tabular-nums;white-space:nowrap}
 ...
 .panel{background:var(--surface);border:1px solid var(--line);border-radius:14px;padding:1.4rem 1.3rem;color:var(--muted);font-size:14px}
...
def _page(inner: str) -> str:
    return (
        ...
        "<title>Trader Goblins &middot; Live account</title>"
...
def render_live_account() -> str:
    ...
    if not d.get("available"):
        why = ("Add <code>ALPACA_API_KEY_ID</code> and <code>ALPACA_API_SECRET_KEY</code> "
               "to the environment to show the live paper account."
               if d.get("error") == "no_keys"
               else f"Couldn't reach Alpaca right now ({_esc(d.get('error'))}). Try a refresh.")
        return _page(
            "<h1>&#128202; Live paper account</h1>"
            "<p class='sub'>The serious paper-trading account, live from Alpaca.</p>"
            f"<div class='panel'>{why}</div>"
        )
    ...
    table = (
        "<table><thead><tr><th>Symbol</th><th>Qty</th><th>Avg entry</th><th>Current</th>"
        "<th>Mkt value</th><th>Unreal P/L</th><th>%</th><th>Today</th></tr></thead>"
        f"<tbody>{rows}</tbody></table>"
        if positions else "<div class='panel'>No open positions.</div>"
    )

    return _page(
        "<h1>&#128202; Live paper account</h1>"
        "<p class='sub'>The serious paper-trading account, live from Alpaca &mdash; reload to refresh.</p>"
        "<div class='hero'>"
        f"<div class='eqlabel'>Equity</div><div class='eq'>{_money(equity)}</div>"
        f"<div class='day {_cls(day_chg)}'>{_money(day_chg)} ({_pct(day_pct)}) today</div>"
        "</div>"
        f"<div class='cards'>{cards_html}</div>"
        f"<h2>Positions</h2>{table}"
